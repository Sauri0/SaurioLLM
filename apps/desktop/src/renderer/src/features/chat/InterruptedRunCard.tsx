...
export function InterruptedRunCard({ info, onRecover, onDismiss }: InterruptedRunCardProps): React.JSX.Element {
  return (
    <div className="interrupted-run-card">
      ...
      <div className="interrupted-run-card__actions">
        <button type="button" onClick={() => onRecover(info.runId)}>Reanudar</button>
        <button type="button" onClick={() => onDismiss(info.runId)}>Descartar</button>
      </div>
    </div>
  );
}
